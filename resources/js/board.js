...
function updateList(jsonObj){
    ...
    if(jsonObj.status == "OK"){
        ...
        window.page = jsonObj.page; // 1
        window.pageRows = jsonObj.pagerows; // 10 
        ...
        for(var i = 0; i < count; i++){
            ...
            result += "<td>" + items[i].score + "</td>\n";
            result += "<td>" + items[i].prfname + "</td>\n";
            result += "</tr>\n";
        }
        ...
        // [페이징 정보 업데이트] // writepages: 컨트롤러에서 설정된 페이지 번호 몇개까지 띄울까?
...
function setPopup(mode){
	...
	if(mode == "write"){	
		...
		$("#dlg_write input[name='prfname']").attr("readonly", false);
		$("#dlg_write input[name='prfname']").css("border", "1px solid #ccc");
		$("select[name='score']").show();
		
		// '찾아보기' 버튼 누르면 팝업
   		$("#findperf").click(function(){
	        setPopup("find");  // 글작성 용으로 모달팝업 셋업
	        $("#dlg_write1 .title").text("연극 찾기");
	...
	if(mode == "view"){
		...
		$("#scoretext").text(viewItem.score);
		$("#scoretext").show();
				
		$("select[name='score']").hide();
		$("#findperf").hide();
		$("#dlg_write input[name='prfname']").val(viewItem.prfname);
		$("#dlg_write input[name='prfname']").attr("readonly", true);
		$("#dlg_write input[name='prfname']").css("border", "none");
	...
	if(mode == "update"){
		...
		$("#dlg_write input[name='prfname']").css("border", "1px solid #ccc");
		$("#scoretext").hide();
		$("select[name='score']").val(viewItem.score);
		$("select[name='score']").show();
